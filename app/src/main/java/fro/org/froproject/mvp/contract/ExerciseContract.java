...
public interface ExerciseContract {
    ...
    interface View extends IView {
        ...
        void setList(List<ExerciseBean> exerciseBean);
    }

    //Model层定义接口,外部只需关心model返回的数据,无需关心内部细节,及是否使用缓存
    interface Model extends IModel {

        Observable<BaseJson<List<ExerciseBean>>> getQuestionList(int classId, int courseId);

        Observable<BaseJson<ExerciseResponseBean>> commit(int classId, int courseId, Map<String, Object> resultMap);
    }
}