...
import fro.org.froproject.mvp.ui.view.StatusBarView;
...
public class StatusBarUtil {
    ...
    public static void setColorAlpha(Activity activity, int color, int statusBarAlpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            activity.getWindow().setStatusBarColor(calculateStatusColor(color, statusBarAlpha));
            ...
            int count = decorView.getChildCount();
            if (count > 0 && decorView.getChildAt(count - 1) instanceof StatusBarView) {
                decorView.getChildAt(count - 1).setBackgroundColor(calculateStatusColor(color, 0));
                decorView.getChildAt(count - 1).getBackground().setAlpha(statusBarAlpha);
            } else {
...
//        layoutParams.setMargins(0, getStatusBarHeight(activity), 0, 0);
    ...
    private static StatusBarView createStatusBarView(Activity activity, int color) {
        // 绘制一个和状态栏一样高的矩形
        StatusBarView SAStatusBarView = new StatusBarView(activity);
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
        SAStatusBarView.setLayoutParams(params);
        SAStatusBarView.setBackgroundColor(color);
        return SAStatusBarView;
    }
    ...
    private static StatusBarView createStatusBarView(Activity activity, int color, int alpha) {
        ...
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
        SAStatusBarView.setLayoutParams(params);
        SAStatusBarView.setBackgroundColor(calculateStatusColor(color, alpha));
        return SAStatusBarView;
    }
    ...
    private static StatusBarView createTranslucentStatusBarView(Activity activity, int alpha) {
        ...
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, getStatusBarHeight(activity));
        SAStatusBarView.setLayoutParams(params);
        SAStatusBarView.setBackgroundColor(Color.argb(alpha, 0, 0, 0));
        return SAStatusBarView;
    }
